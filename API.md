...
Публичный просмотр не требует входа: `GET`-запросы к лошадям, жокеям, командам, соревнованиям, заездам, составам заездов и результатам доступны зрителю.

Изменение данных (`POST`, `PATCH`, `DELETE`) требует входа в систему. Раздел `/api/users` доступен только администратору. Авторизация хранится в HttpOnly cookie, браузер отправляет её сам.
